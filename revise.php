<?php
	...
	if($link) {
		$request_uuid = substr($link, 0, 36);
		$request_approver_uuid = substr($link, 37, 36);
	} else {
...
<html xmlns="http://www.w3.org/2001/XMLSchema">
	<head>
		...
		<title>Ulaşım ve Konaklama - Talep Revize Formu</title>
		...
		<div id="div_main_area">
			<div id="div_main_frame" class="main_frame">
				...
				<div id="div_form_page" align="center">
					<form id="form1" method="post" enctype="multipart/form-data" action="">
						<div hidden>
							...
							<input type="hidden" id="inp_request_approver_uuid" name="inp_request_approver_uuid" value="<?php echo $request_approver_uuid; ?>" />
						</div>
						<div align="left" style="height: 116px;">
							<div id="div_lbl" class="lbl_norm2" alert="lbl_alrt2">Açıklama: <font style="color: red;">*</font></div>
							<div><textarea id="txt_explanation" name="txt_explanation" label="div_lbl" area="area4" action="go" class="inp" style="height: 88px;" placeholder="Revize nedenini yazınız"></textarea></div>
						</div>
						<div align="right">
							<div style="display: flex; width: 195px;">
								<div>
									<input type="button" id="btn_close" class="btn" value="İptal ✘" onClick="window.close();" />
								</div>
								<div style="width: 15px;"></div>
								<div>
									<input type="button" id="btn_ok" area="area4" action="warn" class="btn" value="Tamam ✔" onClick="revise($(this));" />
								</div>
							</div>
						</div>
					</form>		
				...
				<div id="div_completion_page" align="center" hidden>
					...
					<div>Ulaşım ve Konaklama Revize Talebi oluşturuldu.</div>
					<div style="height: 20px;"></div>
					<div align="center">
